...
---Consulta 10 pero utilizando with para las subconsultas
WITH ALIAS_SUBCONCULTA1 AS (
    --SUBCONSULTA
),
ALIAR_SUBCONSULTA2 AS (
    --SUBCONSULTA
)
SELECT *
FROM SUBCONSULTA1
INNER JOIN SUBCONSULTA2
ON. ... ;
------------------------------------------------
...
LEFT JOIN AMIGOS D
ON (A.CODIGO_USUARIO = D.CODIGO_USUARIO)
LEFT JOIN LIKE_PUBLICACIONES E
ON (A.CODIGO_USUARIO = E.CODIGO_USUARIO)
LEFT JOIN LIKE_FOTOS F
ON (A.CODIGO_USUARIO = F.CODIGO_USUARIO)
...
---SIEMPRE CON WITH PERO CON PRODUCTOS CARTESIANOS
...
--NECESITA EL PRIVILEGIO: GRANT create materialized view TO FACEBOOK;
...
--POR SI CAMBIA LOS VALORES EN ALGUNA DEPENDENCIA ES NECESARIO ACTUALIZARLA
BEGIN
    DBMS_MVIEW.REFRESH ('MVW_USUARIOS');
END;